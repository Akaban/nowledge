...
class GKeepList extends React.Component {
  constructor(props) {
    super(props);
  ...
  toggleItem(id) {
    const { externalSetState } = this.props;
    ...
    if (externalSetState)
      externalSetState({items: updatedItems})
  }
  addItem(name) {
    const { externalSetState } = this.props;
  ...
  render() {
    const meta = this.getMeta();
    const { isSimpleList, setDirty } = this.props;
    ...
    const items = this.state.items.map((item, index) => (
      <Item
        ...
        toggleItem={isSimpleList ? () => {} : () => this.toggleItem(item.id)}
        editItem={this.editItem}
        deleteItem={() => this.deleteItem(item.id)}
        setItemHeight={this.setItemHeight}
        isSimpleList={this.props.isSimpleList}
      />
    ));
      ...
      <div id="app">
        <div id="items-outer-container">
          <div id="items-container" className="scroll-bar">
            <AddItemInput addItem={this.addItem} setDirty={setDirty} />
            <div id="items">
              <div
...
class Item extends React.Component {
  ...
  render() {
    ...
    const itemName = isEditing ? (
      ...
      <div
        className="item-name"
        onDoubleClick={!isCompleted ? this.toggleEdit : null}
      >
        <h1>{name}</h1>
      </div>
    );
